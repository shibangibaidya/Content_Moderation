...
Defining a function named extract_all_video_frames() to extract all the video frames and saving them in the extract_frames folder.
...
def extract_all_video_frames(video_root_dir, output_root_dir, frame_skip=10): # Extract frames skipping every 10 frames
    for label_folder in ["Violence", "NonViolence"]: # Loop through both class folders
        ...
        for filename in os.listdir(input_folder):
            if filename.lower().endswith((".avi", ".mp4")):  # now handles both formats
...
The train_one_epoch() function trains the model for one epoch using the training dataloader by performing forward passes, computing loss, backpropagating gradients, and updating model weights. It returns the average loss and accuracy for the epoch.
"""
...
#     return HTML("""
#     <div style="padding: 20px; background-color: #ffdddd; border: 2px solid red; border-radius: 10px; text-align: center; font-size: 18px;">
