# -*- coding: utf-8 -*-
"""Violent_Content_Detection_Single_Frame_Processing.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1CaixNafhP5aD5syfcSZ_McaE627tUHsW
...
"""# Step 3
# Extracting All The First Frames From The Video
Defining a function named extract_all_video_frames() to extract only the first video frames and saving them in the extract_frames folder.

Experimenting with the first frame
"""

def extract_first_frame(video_root_dir, output_root_dir):
        ...
        for filename in os.listdir(input_folder):
            if filename.lower().endswith((".avi", ".mp4")):
                ...
                if ret:
                    ...
                    frame_name = f"{base}_frame0.jpg"
                    cv.imwrite(os.path.join(output_folder, frame_name), frame)
                    print(f"Saved first frame from {filename} → {frame_name}")
                cap.release()

video_dir = "/content/real life violence situations/Real Life Violence Dataset"
output_dir = "/content/extracted_frames"

# extract_all_video_frames(video_dir, output_dir, frame_skip=10)
extract_first_frame(video_dir, output_dir)
...
sample_path = "/content/extracted_frames/NonViolence/NV_106_frame0.jpg"
...
This dataset class loads a video frame, applies basic PyTorch transforms (resizing and tensor conversion), then converts the tensor back to a NumPy array in [0, 255] uint8 format.
"""
...
The train_two_epoch() function trains the model for two epochs
"""
...
This function infer_video() performs frame-by-frame prediction on a video using a trained model.
"""
...
# Loop through each video and run inference on first 1 frame
...
This code randomly selects a video from the dataset, performs inference using the trained model, and then either blocks the video (if predicted violent) or plays it inline in the notebook (if predicted as non-violent) .
"""
...
print(f"Average Inference Time per Video: {avg_inference_time_per_video} ms")
...
print(f"Average Inference Time per Frame: {avg_inference_time_per_frame} ms")
